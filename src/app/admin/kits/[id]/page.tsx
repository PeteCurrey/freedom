...
import { 
  ...
  CheckCircle2, Box, Info, Image as ImageIcon,
  Tag, Loader2, Package
} from "lucide-react";
...
export default function KitDesignerEdit() {
  ...
  // Fetch initial kit data & available products
  useEffect(() => {
    async function loadData() {
      ...
      if (kitId) {
        ...
        if (kitData) {
          setKitName(kitData.name || "");
          setKitSlug(kitData.slug || "");
          setDescription(kitData.description || "");
          setKitPriceGbp(kitData.kit_price_gbp || "");
          setStatus(kitData.status || 'draft');
          setImageUrl(kitData.image || "");

          // In a real app, we'd fetch from `build_kit_items` table and join with products.
          // For mockup purposes, we'll populate 1 random product if none found just to show UI
          // if we can't find real items.
        }
      }
  ...
  const handleSave = async (publish: boolean) => {
    ...
    if (kitError) {
      alert("Error updating kit: " + kitError.message);
    }
  ...
  if (initialLoading) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center space-y-4">
        ...
        <span className="font-mono text-[10px] text-slate-500 uppercase tracking-widest">Loading Kit Configuration...</span>
      </div>
    );
  }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-slate-200">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="font-mono text-[10px] text-slate-500 uppercase tracking-widest mt-1">Editing: {kitName}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            {loading ? "Processing..." : <><CheckCircle2 size={14} /> Update Kit</>}
          </button>
        </div>
      </div>
